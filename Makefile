...
PNN_EVENT_WITH_HIT_FEATURES_FILES_EXTRA = \
...
PNN_EVENT_WITH_HIT_FEATURES_FILES_EXTRA_2021A_AA = \
...
PNN_EVENT_WITH_HIT_FEATURES_FILES_EXTRA_2021A_AB = \
...
PNN_EVENT_WITH_HIT_FEATURES_FILES_EXTRA_2021A_AC = \
...
data-run-events : $(RUN_EVENT_FILE)

data-run-hits : $(RUN_HIT_FILE)

data-run-event_with_hit_features : \
	$(RUN_EVENT_WITH_HIT_FEATURES_FILES) \
	$(RUN_EVENT_WITH_HIT_FEATURES_FILES_EXTRA)

data-run-sample_event_ids : $(RUN_SAMPLE_EVENT_ID_FILES)
...
data-pnn-events : $(PNN_EVENT_FILE)

data-pnn-hits : $(PNN_HIT_FILE)

data-pnn-event_with_hit_features : \
	$(PNN_EVENT_WITH_HIT_FEATURES_FILES) \
	$(PNN_EVENT_WITH_HIT_FEATURES_FILES_EXTRA)
...
xgboost :
	$(PYTHON) scripts/xgb_train.py
	$(NBCONVERT) --to notebook --inplace --execute models/XGBRegressor/XGBRegressor_performance_analysis.ipynb
	$(NBCONVERT) --to notebook --inplace --execute models/XGBRegressor/XGBRegressor_shap_analysis.ipynb

mlp :
	$(PYTHON) scripts/mlp_train.py
	$(PYTHON) scripts/qmlp_train.py
	$(NBCONVERT) --to notebook --inplace --execute models/MLP/mlp_regression_model.ipynb
...
$(PNN_EVENT_WITH_HIT_FEATURES_FILES) : $(PNN_EVENT_WITH_HIT_FEATURES_FILES_2021A_AA) $(PNN_EVENT_WITH_HIT_FEATURES_FILES_2021A_AB) $(PNN_EVENT_WITH_HIT_FEATURES_FILES_2021A_AC)
